...
$(function() {
	...
	function reduirePanneau(){
		...
		$('#panneau2').hide();
		$('#panneau .pane').remove();
		var h = $('#interface .en-tete').height() + 20;
		console.log(h);
...
*/
	...
	if ( $('body').hasClass('single-table') ) {
		...
		$(function(){
			...
			if ( have_modification_right() ) {
				$.contextMenu({
					selector: '.ligne-handler', 
					callback: function(key, options) {
						if ( key == 'supprimer' ) {
							$(this).parent().remove();
							enregistrer();
						} else if (key == 'fleche'){
							if ( $(this).parent().find('path').css("marker-end") != "none" ){
								$(this).parent().find('path').css("marker-end", "none");
							} else {
								$(this).parent().find('path').css("marker-end", "url(#arrow)");
							}
							enregistrer()
						} else if ( key == '0' || key == '1' ||key == '2' ||key == '3' ||key == '4' ) {
							...
							$(this).parent().find('path').css("stroke", couleur[key] );
							$(this).parent().find('marker polyline').attr("fill", couleur[key] );
							enregistrer()
						} else if ( key.substring(0,9) == 'epaisseur'){
							var epaisseur = new Array();
							epaisseur['epaisseur0'] = 1;
							epaisseur['epaisseur1'] = 4;
							$(this).parent().find('path').css("stroke-width", epaisseur[key] );
							enregistrer()
						} 
					},
					items: {
			...
			if ( have_modification_right() ) {
				...
				$('.ligne-wrapper').each( function(){
					lignes.push({
						...
						width 		: parseInt( $(this).css('width')												),
						zindex		: parseInt( $(this).css('z-index')												),
						epaisseur	: $(this).children('svg').children('path').css('stroke-width')									,
						pathString 	: $(this).children('svg').children('path').attr('d')											,
						couleur 	: $(this).children('svg').children('path').css('stroke')
					});
				});
		...
		$("#ecran").mousewheel(function(event, delta) {
			if(delta > 0) {
				...
				if (echelle>maxZoom){ echelle=maxZoom; } else if (echelle < minZoom){ echelle = minZoom; }
		...
		function initLine(){
			if ( have_modification_right() ) {
				$('.ligne-wrapper').draggable({ scroll:false, cursor:"move", handle:'.ligne-handler',
		...
		function addpoint(x, y){
			...
			if (points.length==3){
				...
				var svg  = '<div class="ligne-wrapper" style="position:absolute;width:'+( (Xmax-Xmin) + 2*marge)+'px;height:'+( (Ymax-Ymin) + 2*marge)+'px;left:'+(Xmin)+'px;top:'+(Ymin)+'px;">';
		...
		*/
		...
		$( " .note, .element" ).draggable( "disable" );
